...
uid=`docker run --name nginx_be1  -v /home/brijesh/nginx/index_be1.html:/usr/share/nginx/html/index.html:ro  -d nginx`
ip1=`docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $uid`
echo "BE1 IP: $ip1"

echo "Lanching Backend2"
uid=`docker run --name nginx_be2  -v /home/brijesh/nginx/index_be2.html:/usr/share/nginx/html/index.html:ro -d nginx`
...
uid=`docker run --name nginx_lb -v /home/brijesh/nginx/nginx_rproxy.conf:/etc/nginx/nginx.conf:ro -d nginx`
